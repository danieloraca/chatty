...
struct AppState {
    ...
    chat_history: Arc<Mutex<HashMap<String, Vec<ChatCompletionRequestMessage>>>>,
}
...
async fn main() {
    ...
    let processed_events = Arc::new(Mutex::new(HashSet::new()));
    let chat_history = Arc::new(Mutex::new(HashMap::new()));
    let state = AppState {
        client,
        slack_client,
        processed_events,
